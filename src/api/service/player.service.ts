...
import { CreatePlayerSchema, UpdatePlayerSchema } from '../schema/player.schema';
...
export class PlayerService {
    ...
    update = async (data: IUpdatePlayer) => {
        const { id, nickname, steamId, firstName, lastName, avatar, country, teamId } = UpdatePlayerSchema.parse(data);

        const player = await this.repository.findOneOrError(id);

        const team = teamId ? await this.teamService.getOne(teamId) : null;

        const avatarFileName = await FileService.saveImage(avatar, player.avatar);
